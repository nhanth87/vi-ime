...
pub trait AppPlugin: Send + Sync {
    ...
    fn post_process_action(&mut self, action: NonPreeditAction) -> NonPreeditAction { action }
    fn recommended_mode(&self, _app_id: &str) -> Option<crate::engine::ImeMode> { None }
}
...
impl PluginManager {
    pub fn new() -> Self { Self { plugins: Vec::new(), current_app: None } }
    ...
    pub fn on_focus_change(&mut self, app_id: &str) {
        if let Some(ref old) = self.current_app {
            for p in &mut self.plugins {
                if p.handles_app(old) { p.on_blur(old); }
            }
        }
    ...
    pub fn pre_process_key(&mut self, ch: char, system_mod: bool, app_id: Option<&str>) -> Option<NonPreeditAction> {
    ...
    pub fn post_process_action(&mut self, mut action: NonPreeditAction, app_id: Option<&str>) -> NonPreeditAction {
        let app = app_id.unwrap_or("");
        for p in &mut self.plugins {
            if p.handles_app(app) { action = p.post_process_action(action); }
        }
        action
    }
    ...
    pub fn recommended_mode(&self, app_id: &str) -> Option<crate::engine::ImeMode> {
        for p in &self.plugins {
            if p.handles_app(app_id)
                && let Some(m) = p.recommended_mode(app_id) { return Some(m); }
        }
        None
    }
}
...
impl AppPlugin for TerminalPlugin {
    ...
    fn pre_process_key(&mut self, _ch: char, _m: bool) -> Option<NonPreeditAction> { None }
...
impl AppPlugin for BrowserPlugin {
    fn name(&self) -> &str { "BrowserPlugin" }
    fn handles_app(&self, id: &str) -> bool {
...
pub struct ChromiumNiriPlugin { is_niri: bool }
impl ChromiumNiriPlugin {
    pub fn new() -> Self {
        let is_niri = std::env::var("XDG_CURRENT_DESKTOP")
            .map(|d| d.to_lowercase().contains("niri")).unwrap_or(false);
        Self { is_niri }
    }
}
impl AppPlugin for ChromiumNiriPlugin {
    fn name(&self) -> &str { "ChromiumNiriPlugin" }
    fn handles_app(&self, id: &str) -> bool {
        if !self.is_niri { return false; }
        let id = id.to_lowercase();
        id.contains("chrom") || id.contains("chrome") || id.contains("brave")
            || id.contains("edge") || id.contains("opera") || id.contains("vivaldi")
    }
    fn on_focus(&mut self, id: &str) {
        tracing::info!("[ChromiumNiriPlugin] {}: forcing NonPreedit to prevent double-input", id);
    }
    fn on_blur(&mut self, _id: &str) {}
    fn pre_process_key(&mut self, _ch: char, _m: bool) -> Option<NonPreeditAction> { None }
    fn recommended_mode(&self, _app_id: &str) -> Option<crate::engine::ImeMode> {
...
pub struct AutoCommitShortcutPlugin { state: ShortcutState }
impl AutoCommitShortcutPlugin {
    pub fn new() -> Self { Self { state: ShortcutState::default() } }
}
impl AppPlugin for AutoCommitShortcutPlugin {
    fn name(&self) -> &str { "AutoCommitShortcut" }
    fn handles_app(&self, _id: &str) -> bool { true }
    fn on_focus(&mut self, _id: &str) {}
    fn on_blur(&mut self, _id: &str) { self.state = ShortcutState::default(); }
    fn pre_process_key(&mut self, _ch: char, system_mod: bool) -> Option<NonPreeditAction> {
        if system_mod && self.state.has_pending {
            ...
            tracing::info!("[AutoCommitShortcut] auto-commit pending \"{}\" before shortcut", text);
            return Some(NonPreeditAction::CommitWithBackspace { backspace_count: raw, text });
        }
        None
    }
}
...
impl ElectronFlagAdvisorPlugin {
    pub fn new() -> Self { Self { warned: std::collections::HashSet::with_capacity(16) } }
}
impl AppPlugin for ElectronFlagAdvisorPlugin {
    fn name(&self) -> &str { "ElectronFlagAdvisor" }
    fn handles_app(&self, id: &str) -> bool {
        let id = id.to_lowercase();
        id.contains("discord") || id.contains("slack") || id.contains("teams")
            || id.contains("capcut") || id.contains("code") || id.contains("codium")
            || id.contains("vscode") || id.contains("sublime") || id.contains("subl")
            || id.contains("telegram") || id.contains("signal") || id.contains("element")
            || id.contains("spotify") || id.contains("notion") || id.contains("figma")
    }
    fn on_focus(&mut self, id: &str) {
        if self.warned.insert(id.to_string()) {
            tracing::warn!(
                "[ElectronFlagAdvisor] {} is Electron/Chromium. Add flags: --ozone-platform=wayland --enable-wayland-ime", id);
        }
    }
    fn on_blur(&mut self, _id: &str) {}
    fn pre_process_key(&mut self, _ch: char, _m: bool) -> Option<NonPreeditAction> { None }
