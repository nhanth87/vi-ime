...
~~Chromium-family trên niri resolve NonPreedit ở layer builtin~~ — **ĐÃ
ĐẢO NGƯỢC 2026-07-10 khuya, đọc kỹ:** flip NonPreedit đẩy Chrome vào live
path và Blink áp `wl_keyboard.keymap` trễ vô hạn định (KHÔNG pacing nào
cứu nổi — 15ms beat vẫn ra "tu72 dau61 tie6m5" → "phò từ gâu gâu6m5",
repro textarea file:// + uinput). Chrome + Preedit trên niri gõ hoàn hảo
("từ dấu tiệm bây giờ ừ") — nghi án "Preedit double-input dưới niri" khi
xưa gần như chắc là rival `fcitx5_uinput_server` (Tính năng 5). Quy tắc:
**Blink/Electron không bao giờ được rơi vào live path qua builtin**; nếu
user tự set NonPreedit per-app thì tự chịu (user > builtin, R13).
