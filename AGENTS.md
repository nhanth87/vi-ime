...
Chuỗi sự kiện MỘT đêm: (1) flip builtin Chromium→NonPreedit đẩy Chrome
vào live path; Blink áp `wl_keyboard.keymap` trễ VÔ HẠN ĐỊNH — không
pacing nào cứu nổi ("tu72 dau61 tie6m5" → "phò từ gâu gâu6m5", repro
textarea file:// + uinput); cache grow-only còn dồn 'ấ' lên keycode 28 =
KEY_ENTER → "gõ 'mất' là nó tự commit thành dấu enter" (tự gửi message
giữa từ). (2) Vá bằng builtin Chromium→Preedit → user bác ("gạch đít cho
non-preedit") vì builtin đè toggle global (R13). (3) Vá bằng buffer-âm-
thầm → user bác tiếp ("gõ k hiện chữ cho tới khi commit"). (4) **KIẾN
TRÚC CUỐI — giải đúng ràng buộc gốc (hiện ngay + không gạch chân + Blink
không nuốt): viet_typer dùng KEYMAP TĨNH 8-LEVEL.**
- **Keymap upload MỘT LẦN lúc tạo typer, KHÔNG BAO GIỜ đổi nữa** — Blink
  áp trễ cũng chỉ trễ một lần trước từ đầu tiên. 36 keycode an toàn
  (`SAFE_CODES` — hàng phím chữ/số, LOẠI 14=BS/15=Tab/28=Enter/29=Ctrl,
  không bao giờ gán ký tự vào 4 code đó dù keymap có remap) × 8 level
  (type VIIM: Shift/Mod3/Mod5) = 280 slot ≥ toàn bộ ASCII + chữ Việt
  hoa/thường. Level chọn bằng `vk.modifiers(depressed)` ngay trước tap,
  cùng object → ordering protocol-guaranteed. Unit test
  `static_keymap_covers_engine_output` compile keymap bằng xkbcli thật.
- **`live_echo()` (state.rs) là predicate DUY NHẤT** cho live mode
  (= NonPreedit && viet.ready()), 6 chỗ từng inline đều gọi nó. Live giờ
  an toàn trên MỌI app (verify: kitty + Chrome + Electron, rollover
  20ms/phím, hoa có dấu VIỆT, không tự-Enter). Buffer-âm-thầm +
  commit_string chỉ còn là fallback khi không có virtual keyboard.
- **BUILTIN_APPS đã GỠ hết entry Preedit** (browsers/chat): builtin đè
  global nên mỗi entry là một app mà toggle user chết. Chỉ giữ
  NonPreedit cho editor/IDE. Nghi án "Preedit double-input dưới niri"
  khi xưa gần như chắc là rival `fcitx5_uinput_server` (Tính năng 5).
- **`scripts/vi-regression/run.sh` = bộ regression field-level** (uinput
  seq + rollover, zenity preedit + kitty live, assert cả nội dung lẫn
  không-tự-Enter; `CHROME=1` thêm bài Blink). CHẠY TRƯỚC KHI SHIP mọi
  thay đổi typing path — mọi regression 2026-07-10 đều bị battery này
  bắt được.
